...
async function handleRequestEvaluation() {
    ...
    // 若已终末锁定且正在加载，直接显示加载态
    if (current.isFinalized && current.isLoadingEvaluation) {
        showEvaluationModal(true, null);
        return;
    }
    // 阶段二 2.1：立即终末锁定与进入加载态
    stateManager.updateState({ isFinalized: true, isLoadingEvaluation: true, isEvaluating: true, evaluationError: null, isExperimentLocked: true });
    // 锁UI
    try { import('./ui-components/ControlPanel.js').then(m => m.lockAllControls && m.lockAllControls()); } catch(e) {}
    showEvaluationModal(true, null);
    ...
    console.log('📊 收集评价数据:', { operationLog: operationLog.length, userFeedback: userFeedback.length, qaHistory: qaHistory.length });

    try {
        const result = await evaluateExperiment(operationLog, userFeedback, qaHistory);
        // 阶段二 2.4：成功后写入评价结果并关闭加载态
        stateManager.updateState({ evaluationResult: result, isLoadingEvaluation: false });
        showEvaluationModal(true, result);
    } catch (err) {
        stateManager.updateState({ evaluationError: err.message || String(err), isLoadingEvaluation: false });
...
async function handleSendMessage(action) {
    ...
    if (action.type === 'send_message') {
        ...
        // 立即锁定UI
        try {
            import('./ui-components/ControlPanel.js').then(mod => {
                if (mod && mod.lockAllControls) mod.lockAllControls();
            });
        } catch (e) {}
        
        // V3.3: 门控逻辑 - 如果之前是锁定状态，现在解锁（保持锁定，不再解锁）
        if (isCurrentlyLocked) {
            stateManager.updateState({ isExperimentLocked: false });
            console.log('V3.3: 用户提交反馈，门锁已解锁');
        }
...
function updateAllUI(state) {
    // 阶段二 2.2：若终末锁定，强制控件禁用
    if (state.isFinalized) {
        try {
            import('./ui-components/ControlPanel.js').then(m => m.lockAllControls && m.lockAllControls());
        } catch (e) {}
    }
