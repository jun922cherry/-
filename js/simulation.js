...
// V2.2: 碰撞监视器数据记录器（模块级别变量）
let collisionRecorder = null;

// V2.2: 当前模拟状态（用于时间尺度计算）
...
const SIMULATION_CONFIG = {
    ...
    particleRadius: 4,    // V2.2.5: 减小分子半径以适应更多分子
    pistonThickness: 15,  // FIX: 减小活塞厚度避免超出显示区域
    wallThickness: 15     // FIX: 减小墙体厚度
};
...
function initPhysicsEngine(initialState) {
    ...
    // 重新计算体积映射范围，确保在可视区域内
    // 考虑墙体厚度和活塞厚度，留出更大的安全边距
    const leftMargin = SIMULATION_CONFIG.wallThickness + 30; // 左侧墙 + 更大安全距离
    const rightMargin = SIMULATION_CONFIG.wallThickness + SIMULATION_CONFIG.pistonThickness + 20; // 右侧墙 + 完整活塞 + 更大安全距离
    VOLUME_MAPPING.minX = leftMargin;  // 活塞最左位置
    VOLUME_MAPPING.maxX = SIMULATION_CONFIG.width - rightMargin; // 活塞最右位置
    
    console.log(`📐 活塞位置映射范围: minX=${VOLUME_MAPPING.minX}, maxX=${VOLUME_MAPPING.maxX}, 可用宽度=${VOLUME_MAPPING.maxX - VOLUME_MAPPING.minX}px`);
    ...
    // V2.2.5: 时间尺度显示已移至HTML元素，不再需要Canvas叠加
    // drawTimeScaleOverlay(ctx, canvas, currentTimeScale);
    
    // V2.2.5+FIX: 确保活塞在显示范围内（使用VOLUME_MAPPING的边界）
    if (walls.right) {
        // 计算墙体的安全边界（墙体位置 = 活塞位置 - 活塞厚度/2）
        const wallMinX = VOLUME_MAPPING.minX - SIMULATION_CONFIG.pistonThickness / 2;
        const wallMaxX = VOLUME_MAPPING.maxX - SIMULATION_CONFIG.pistonThickness / 2;
        
        if (walls.right.position.x > wallMaxX) {
            Matter.Body.setPosition(walls.right, {
                x: wallMaxX,
                y: walls.right.position.y
            });
            console.warn(`⚠️ 活塞超出右边界，已修正至 ${wallMaxX}`);
        } else if (walls.right.position.x < wallMinX) {
            Matter.Body.setPosition(walls.right, {
                x: wallMinX,
                y: walls.right.position.y
            });
            console.warn(`⚠️ 活塞超出左边界，已修正至 ${wallMinX}`);
        }
        
        // FIX: 检查并移除超出活塞右侧的分子
        const pistonLeftEdge = walls.right.position.x - SIMULATION_CONFIG.pistonThickness / 2;
        const particlesToRemove = [];
        particles.forEach(particle => {
            if (particle.position.x > pistonLeftEdge - SIMULATION_CONFIG.particleRadius) {
                particlesToRemove.push(particle);
            }
        });
        if (particlesToRemove.length > 0) {
            Matter.World.remove(world, particlesToRemove);
            particles = particles.filter(p => !particlesToRemove.includes(p));
            console.warn(`⚠️ 移除了 ${particlesToRemove.length} 个超出边界的分子`);
        }
    }
...
/**
 * V2.2: 设置碰撞记录器
 * @param {Function} recorder - 碰撞记录函数
 */
export function setCollisionRecorder(recorder) {
    if (typeof recorder === 'function') {
        collisionRecorder = recorder;
        console.log('碰撞记录器已设置');
    }
}
 ...
 */
function updateTimeScaleDisplay(temperature) {
    const scale = calculateTimeScale(temperature);
    
    const speedSlowdownEl = document.getElementById('speed-slowdown');
    const realSpeedEl = document.getElementById('real-speed');
    
    if (speedSlowdownEl) {
        speedSlowdownEl.textContent = `${scale.mantissa}×10${scale.exponent}`;
    }
    
    if (realSpeedEl) {
        realSpeedEl.textContent = scale.realAvgSpeed;
    }
}

// V2.2.2: 计算时间尺度比例（更精确的物理模型）
function calculateTimeScale(temperature) {
    // 使用麦克斯韦-玻尔兹曼分布计算分子平均速率
    // v_avg = sqrt(8kT/πm)
    ...
    const physicalWidth = 22.4e-3; // 物理宽度约22.4mm（对应22.4L体积的一个截面）
    ...
    // 时间缩放比例 = 模拟速度 / 真实速度
    const timeScale = simulationSpeedReal / realAvgSpeed;
    
    // 速度缩慢比例（真实速度 / 模拟速度）
    const speedSlowdown = 1 / timeScale;
    
    // 返回科学记数法表示
    const exponent = Math.floor(Math.log10(Math.abs(speedSlowdown)));
    const mantissa = speedSlowdown / Math.pow(10, exponent);
    
    return { 
        mantissa: mantissa.toFixed(1), 
        exponent: exponent,
        speedSlowdown: speedSlowdown,
        realAvgSpeed: realAvgSpeed.toFixed(1)
    };
}

// V2.2.2: 绘制速度缩放比例显示叠加层
function drawTimeScaleOverlay(ctx, canvas, timeScale) {
    ...
    // 第一行：速度缩慢比例
    const line1 = `速度缩慢: ${scale.mantissa}×10`;
    const superscript1 = `${scale.exponent}`;
    ctx.font = '12px monospace'; // V2.2.3: 在测量前设置字体
    const line1Width = ctx.measureText(line1).width;
    
    // 第二行：真实分子速度
    const line2 = `真实速度: ${scale.realAvgSpeed} m/s`;
    ctx.font = '11px monospace';
    const line2Width = ctx.measureText(line2).width;
    
    // 计算背景宽度（取最大）
    const maxWidth = Math.max(line1Width + 15, line2Width);
    
    // 绘制半透明背景
    ctx.fillStyle = 'rgba(255, 255, 255, 0.85)';
    ctx.fillRect(textX - maxWidth - 8, textY - 42, maxWidth + 16, 46);
    
    // 绘制边框
    ctx.strokeStyle = 'rgba(100, 116, 139, 0.3)';
    ctx.lineWidth = 1;
    ctx.strokeRect(textX - maxWidth - 8, textY - 42, maxWidth + 16, 46);
    
    // 绘制第一行文本
    ctx.fillStyle = 'rgba(51, 65, 85, 0.95)';
    ctx.font = '12px monospace';
    ctx.fillText(line1, textX - 12, textY - 20);
    
    // 绘制第一行上标
    ctx.font = '9px monospace';
    ctx.fillText(superscript1, textX, textY - 24);
    
    // 绘制第二行文本
    ctx.font = '11px monospace';
    ctx.fillStyle = 'rgba(100, 116, 139, 0.9)';
    ctx.fillText(line2, textX, textY - 4);
...
export function update(currentState) {
    ...
    // V2.2.4: 更新HTML时间比例显示
    updateTimeScaleDisplay(currentState.T);
    ...
    // V2.2: 触发碰撞监视器更新
    if (window.updateCollisionMonitor) {
        window.updateCollisionMonitor(currentState);
    }
}
...
function createParticles(moles, temperature) {
    ...
    // V2.2.5: 增加分子数量（1摩尔 = 50个分子）
    const particleCount = Math.max(1, Math.round(moles * 50));
    ...
    for (let i = 0; i < particleCount; i++) {
        ...
        // FIX: 使用当前活塞实际位置（从墙体位置推算）
        const currentPistonX = walls.right ? (walls.right.position.x + SIMULATION_CONFIG.pistonThickness / 2) : calculatePistonPosition(22.4);
        const maxX = currentPistonX - SIMULATION_CONFIG.pistonThickness/2 - particleRadius - 5;
...
function updateParticleCountAndSpeed(moles, temperature) {
    ...
    const targetCount = Math.round(moles * 50);  // V2.2.5: 增加分子数量
    ...
    if (particles.length < targetCount) {
        ...
        for (let i = 0; i < toAdd; i++) {
            ...
            // FIX: 使用当前活塞实际位置（从墙体位置推算）
            const currentPistonX = walls.right ? (walls.right.position.x + SIMULATION_CONFIG.pistonThickness / 2) : calculatePistonPosition(22.4);
            const maxX = currentPistonX - SIMULATION_CONFIG.pistonThickness/2 - SIMULATION_CONFIG.particleRadius - 5;
