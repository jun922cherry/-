...
export let state = {
    ...
    // V2.EVAL.FINAL: 终末锁定状态（一旦为true，UI全局锁定且停止日志记录）
    isFinalized: false,
...
export function addLogEntry(entryText) {
    // 终末锁定后不再记录日志，防止污染评价数据
    try {
        const s = getState ? getState() : state;
        if (s && s.isFinalized) {
            return;
        }
    } catch (e) {
        // fallback: 若getState不可用，直接检查本地state
        if (state.isFinalized) return;
    }
    const timestamp = new Date().toLocaleTimeString();
    const logEntry = { timestamp, text: entryText, id: Date.now() };
